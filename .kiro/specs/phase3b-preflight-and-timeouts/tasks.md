...
- [ ] 1. Create foundational modules
  - [ ] 1.1 Add an optional `config` parameter to `_make_client()` in `mcp_server/backends/aws_provider.py`
    - Backward compatible: defaults to `None`, every existing call site is unaffected
    - _Requirements: 1.3_

  - [ ] 1.2 Create `core/health.py`
  ...
  - [ ] 1.3 Write unit tests for `core/health.py` (`tests/test_health.py`)
  ...
  - [ ] 1.4 Write property tests for health preflight
  ...
  - [ ] 1.6 Write property test for timeout validation
    - **Property 5: Timeout Validation Partition**
    - **Validates: Requirements 2.3, 2.4**

  - [ ] 1.7 Write unit tests for `core/timeouts.py` (`tests/test_timeouts.py`)
...
- [ ] 3. Wire backend reachability preflight into the Orchestrator
  - [ ] 3.1 Call `check_backend_health()` at the top of `execute_audit()`
  ...
  - [ ] 3.2 Write unit tests for Orchestrator preflight wiring (`tests/test_orchestrator_health_preflight.py`)
  ...
  - [ ] 3.3 Add the readiness indicator to the Streamlit UI
...
- [ ] 4. Implement configurable subprocess/LLM timeouts
  - [ ] 4.1 Replace hardcoded `timeout=` literals in `orchestrator.py` with `get_timeout(...)` calls
  ...
  - [ ] 4.2 Replace `core/llm_client.py`'s hardcoded `_TIMEOUT = 30` with `get_timeout("JANITOR_LLM_TIMEOUT")` read at `get_client()` call time
    - _Requirements: 2.2_

  - [ ] 4.3 Add `subprocess.TimeoutExpired` handling alongside each existing non-zero-exit-code check in `approve()`, `_handle_confirm_rollback()`, `_run_pre_remediation_hook()`, and `_run_post_remediation_hook()`
  ...
  - [ ] 4.4 Write unit tests for timeout-fired handling (`tests/test_timeout_handling.py`)
