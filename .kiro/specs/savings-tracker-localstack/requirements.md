...
3. THE Docker_Compose_Configuration SHALL define a `localstack` service in `docker-compose.yml` (community, pinned to version 4.4.0) that exposes port 4566 bound to 127.0.0.1 only and configures the `SERVICES` environment variable to include EC2 and S3. A separate `docker-compose.pro.yml` override adds ElastiCache and uses the Pro image for full demo support. (Deviation: originally unpinned `latest` image with EBS service; `ebs` is not a valid LocalStack service name — EBS lives under `ec2`. Image pinned for reproducibility. Port bound to localhost per security audit finding L9.)
