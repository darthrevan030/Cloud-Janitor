...
- [x] 1. Create the foundational `StateStore` module
  - [x] 1.1 Implement schema creation, connection setup, and WAL configuration in `core/state_store.py`
  ...
  - [x] 1.2 Implement plans table operations
  ...
  - [x] 1.3 Implement pending_rollbacks table operations
  ...
  - [x] 1.4 Implement audit_trail table operations
  ...
  - [x] 1.5 Write property tests for `StateStore` (foundational)
  ...
  - [x] 1.6 Write unit tests for schema and bootstrap (`tests/test_state_store_schema.py`)
...
- [x] 3. Add `StateStore` corruption and fail-safe handling
  - [x] 3.1 Implement `StateStoreCorruptedError` fail-closed path
  ...
  - [x] 3.2 Wrap all read/write methods with the documented fail-safe error handling
  ...
  - [x] 3.3 Write unit tests for corruption and fail-safe behavior (`tests/test_state_store_corruption.py`)
  ...
  - [x] 3.4 Implement `StateStoreUnavailableError` and distinguish it from corruption (CRITICAL regression test)
...
- [x] 4. Add WAL concurrency verification
  - [x] 4.1 Write a concurrency integration test harness
...
- [x] 6. Wire `StateStore` into the Orchestrator for plans
  - [x] 6.1 Add `STATE_STORE_PATH` to `core/paths.py` and construct `StateStore` in `Orchestrator.__init__`
  ...
  - [x] 6.2 Replace plan-write and plan-read call sites
  ...
  - [x] 6.3 Write property test for cross-process plan visibility
    - **Property 2: Cross-Process Plan Visibility**
    - **Validates: Requirements 2.2, 2.4**

  - [x] 6.4 Write unit tests for plan persistence wiring (`tests/test_orchestrator_plan_persistence.py`)
    - `execute_audit()` followed by `approve()` on a _second_ `Orchestrator` instance pointed at the same output directory finds the plan and proceeds
...
- [x] 7. Wire `StateStore` into the Orchestrator for pending rollbacks
  - [x] 7.1 Replace pending-rollback call sites in `rollback()` and `_handle_confirm_rollback()`
  ...
  - [x] 7.2 Write property test for pending rollback persistence
    - **Property 3: Pending Rollback Idempotence and Restart Survival**
    - **Validates: Requirements 3.4, 3.5**

  - [x] 7.3 Write unit tests for rollback persistence wiring (`tests/test_orchestrator_rollback_persistence.py`)
    - `ROLLBACK <id>` on one `Orchestrator` instance, then `CONFIRM ROLLBACK <id>` on a _second_ instance pointed at the same output directory (restart simulation) succeeds
...
- [x] 9. Wire `StateStore` into the Orchestrator for the audit trail
  - [x] 9.1 Extend `_log_action()` to write to the StateStore alongside the existing `audit.log` write
  ...
  - [x] 9.2 Rewire `get_audit_trail()` to read from the StateStore, with in-memory fallback on read failure
  ...
  - [x] 9.3 Write unit tests for audit trail persistence wiring (`tests/test_orchestrator_audit_persistence.py`)
    - `_log_action()` writes appear in both `audit.log` and `get_audit_trail()`'s output
    - `get_audit_trail()` on a _second_ `Orchestrator` instance (restart simulation) includes entries logged by the first instance
...
- [x] 10. Final integration and cleanup pass
  - [x] 10.1 Confirm no remaining control-flow reads of `_last_plans`/`_pending_rollbacks`/`_audit_trail`
  ...
  - [x] 10.2 Write a wiring-integrity unit test (`tests/test_orchestrator_state_store_wiring.py`)
