...
- [x] 1. Create the least-privilege IAM policy artifacts
  - [x] 1.1 Create `iam/` directory and `iam/janitor-read-policy.json`
  ...
  - [x] 1.2 Create `iam/janitor-remediation-policy.json`
  ...
  - [x] 1.3 Write policy-artifact static validity tests (`tests/test_iam_policies.py`)
  ...
  - [x] 1.4 Write Property 4 & 5 tests for the read policy (`tests/test_iam_read_policy.py`)
  ...
  - [x] 1.5 Write Property 6 & 7 tests for the remediation policy (`tests/test_iam_remediation_policy.py`)
  ...
  - [x] 1.6 Write Property 8 test — cross-check actual rendered HCL, not just the hand-written table (`tests/test_iam_remediation_policy_hcl.py`)
  ...
  - [x] 1.7 Gate `iam/janitor-remediation-policy.json` on real-AWS (or `iam:SimulateCustomPolicy`) validation before merge
...
- [x] 2. Checkpoint — Ensure policy artifact tests pass
...
- [x] 3. Implement remediation-role credential assumption
  - [x] 3.1 Add `_assume_remediation_role()` and `RemediationRoleAssumptionError` to `orchestrator/orchestrator.py`
  ...
  - [x] 3.2 Write property tests for remediation-role assumption
  ...
  - [x] 3.3 Write unit tests for `_assume_remediation_role()` (`tests/test_remediation_role.py`)
...
- [x] 4. Wire remediation-role credentials into the Orchestrator's approve/rollback flow
  - [x] 4.1 Add `_terraform_env_for_apply(resource_id)` to `Orchestrator`
  ...
  - [x] 4.2 Wire `_terraform_env_for_apply()` into `approve()` and `_handle_confirm_rollback()` — **all four `env=_build_subprocess_env("terraform")` call sites**, not just `init`
  ...
  - [x] 4.3 Write unit tests for Orchestrator wiring (`tests/test_orchestrator_remediation_role.py`)
...
- [x] 6. Write the consolidated deployment guide
  ...
  - [x] 6.2 Write content-presence tests for the deployment guide (`tests/test_deployment_docs.py`)
...
- [x] F.1 Fast-follow: fix (or file separately) the `_rollback_security_group` tags-on-unsupported-resource bug
...
- [x] F.2 Follow-up (execute only after `phase1-trust-hardening` ships): update `docs/deployment.md`'s "designed, pending implementation" wording
