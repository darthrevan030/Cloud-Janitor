...
- [x] 1. Create `core/plan_diff.py`
  - [x] 1.1 Implement `PlanChange`, `PlanPreview` dataclasses and `summarize_plan(plan_json)` per the design's action-bucket partition
    - _Requirements: 1.1, 1.2_

  - [x] 1.2 Write property test for plan summary partition completeness
    - **Property 1: Plan Summary Partition Completeness**
    - **Validates: Requirement 1.1**

  - [x] 1.3 Write unit tests for `summarize_plan()` (`tests/test_plan_diff.py`)
...
- [x] 3. Implement `Orchestrator.preview_plan()` and cache wiring
  - [x] 3.1 Extract `approve()`'s inline apply-directory construction (HCL write + provider config) into a reusable `_build_apply_dir(plan)` helper
    - Refactor only — no behavior change to the existing `approve()` path
    - _Requirements: 1.3_

  - [x] 3.2 Implement `Orchestrator.preview_plan(resource_id)` and the `_plan_previews` cache
  ...
  - [x] 3.3 Wire cache reuse into `approve()`
  ...
  - [x] 3.4 Write property tests for preview cache reuse and expiry
  ...
  - [x] 3.5 Write unit tests for preview/approve integration (`tests/test_orchestrator_preview.py`)
  ...
  - [x] 3.6 Add "Preview Plan" / gated "Approve" controls to the Streamlit UI
