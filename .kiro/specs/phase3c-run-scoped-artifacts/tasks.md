...
- [x] 1. Create foundational module
  - [x] 1.1 Create `core/run_context.py`
  ...
  - [x] 1.2 Write property tests for `core/run_context.py`
  ...
  - [x] 1.3 Write unit tests for `core/run_context.py` (`tests/test_run_context.py`)
...
- [x] 3. Implement run-scoped reasoning log (OBS-1)
  - [x] 3.1 Wire `Orchestrator.execute_audit()` to generate/reuse a Run_ID
  ...
  - [x] 3.2 Add `ReasoningLogger.start_new_run(run_id)`, retire the `truncate()` call site
  ...
  - [x] 3.3 Write property test for reasoning log retention
    - **Property 2: Reasoning Log Retention Bound**
    - **Validates: Requirement 2.3**

  - [x] 3.4 Write unit tests for run-scoped reasoning log (`tests/test_reasoning_log_run_scoped.py`)
  ...
  - [x] 3.5 Add reasoning-log listing/viewing to the Streamlit UI
...
- [x] 5. Implement run-scoped findings store (BUG-2)
  - [x] 5.1 Add `FINDINGS_STORE_DIR`, `FINDINGS_STORE_LATEST_POINTER` to `core/paths.py`
  ...
  - [x] 5.2 Wire the Orchestrator to set run-scoped `findings_store_path` on FinOps/SecOps/Architect and update the pointer
  ...
  - [x] 5.3 Write property tests for findings store isolation and pointer safety
    - **Property 3: Findings Store Isolation**
    - **Property 4: Findings Store Pointer Never References a Pruned File**
    - **Validates: Requirements 3.6, 3.7**

  - [x] 5.4 Write unit tests for run-scoped findings store (`tests/test_findings_store_run_scoped.py`)
  ...
  - [x] 5.5 Update `app.py` to read via `resolve_latest_findings_store()` with retry-once-on-`FileNotFoundError`
  ...
  - [x] 5.6 Write unit tests for the retry-once reader (`tests/test_app_findings_reader.py`)
