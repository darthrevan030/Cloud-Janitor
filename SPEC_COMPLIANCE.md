...
Generated: 2026-07-04T10:32:05Z
...
| 14 | 3. Implement persistent approval gates | ✅ Done | "APPROVE" not found in codebase |
...
| 17 | 3.3 Write property tests for approval gate persistence | ✅ Done | "APPROVE" not found in codebase |
...
| 25 | 6. Implement pre-remediation hook full validation | ✅ Done | agents/remediation_architect.py missing |
| 26 | 6.1 Implement `_run_pre_remediation_hook_full()` method | ✅ Done | agents/remediation_architect.py missing |
| 27 | 6.2 Write property tests for pre-remediation hook | ✅ Done | agents/remediation_architect.py missing |
...
| 36 | 8. Implement savings tracker broad exception handling | ✅ Done | agents/savings_tracker.py missing |
| 37 | 8.1 Wrap `savings_tracker.record_run()` with broad exception handling | ✅ Done | agents/savings_tracker.py missing |
| 38 | 8.2 Write property test for savings tracker exception swallowing | ✅ Done | agents/savings_tracker.py missing |
| 39 | 9. Checkpoint - Ensure all tests pass | ✅ Done | no mapping |
| 40 | 10. Implement UI–Orchestrator contract alignment | ✅ Done | app.py missing |
| 41 | 10.1 Refactor `app.py` audit delegation to use public Orchestrator API only | ✅ Done | app.py missing |
| 42 | 10.2 Write unit tests for UI delegation (`tests/test_ui_delegation.py`) | ✅ Done | app.py missing |
...
| 47 | 10.7 Update `app.py` to import paths from `core/paths.py` | ✅ Done | app.py missing |
...
| 52 | 11.3 Surface structured error fields in Streamlit UI | ✅ Done | app.py missing |
...
| 6 | 1. Implement aws_janitor_mcp.py with MCP protocol | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
...
| 10 | 5. Write mcp_server/README.md | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 11 | 1. FinOps Auditor — calls MCP, produces findings[], writes findings_store.json | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 12 | 2. SecOps Guard — calls MCP, appends to findings_store.json | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 13 | 3. Remediation Architect — reads findings, dependency check, generates HCL | ✅ Done | agents/remediation_architect.py missing |
| 14 | 4. Rollback HCL generation (alongside remediation, not after) | ✅ Done | agents/remediation_architect.py missing |
| 15 | 5. findings_store.json schema validation | ✅ Done | output/findings_store.json missing |
| 16 | 1. pre-remediation.sh — terraform validate gate | ✅ Done | agents/remediation_architect.py missing |
| 17 | 2. post-remediation.sh — audit.log append | ✅ Done | agents/remediation_architect.py missing |
...
| 6 | 2. Implement Phase B AI agents (QueryInterpreter, RemediationExplainer, PolicySuggester) | ✅ Done | agents/remediation_architect.py missing |
...
| 10 | 2.4 Write property test for RemediationExplainer schema completeness | ✅ Done | agents/remediation_architect.py missing |
...
| 30 | 8. Wire MCP tools and orchestrator integration | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 31 | 8.1 Add MCP tool `interpret_query` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 32 | 8.2 Add MCP tool `explain_remediation` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 33 | 8.3 Add MCP tool `suggest_policies` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 34 | 8.4 Add MCP tool `infer_resource_context` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 35 | 8.5 Add MCP tool `detect_anomalies` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 36 | 8.6 Add MCP tool `policy_from_incident` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
...
| 2 | 1.1 Create `mcp_server/backends/__init__.py` with CloudProvider abstract base class | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
...
| 6 | 2.1 Implement AWSProvider in `mcp_server/backends/aws_provider.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 7 | 2.2 Implement GCPProvider and AzureProvider in `mcp_server/backends/gcp_provider.py` and `mcp_server/backends/azure_provider.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 8 | 2.3 Update `mcp_server/backends/__init__.py` to export all providers | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 9 | 3. Wire provider selection into MCP server | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 10 | 3.1 Add PROVIDER_REGISTRY and `_load_provider()` to `aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 11 | 3.2 Refactor MCP tool functions to delegate to provider instance | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
...
| 18 | 6.3 Update `mcp_server/README.md` with provider architecture documentation | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
...
| 2 | 1.1 Create `agents/savings_tracker.py` with SavingsTracker class | ✅ Done | agents/savings_tracker.py missing |
| 3 | 1.2 Write property test: RunEntry schema and field correctness | ✅ Done | no mapping |
| 4 | 1.3 Write property test: Monthly savings computation | ✅ Done | agents/savings_tracker.py missing |
...
| 10 | 2.2 Integrate ReasoningLogger into FinOps Auditor | ✅ Done | agents/finops_auditor.py missing |
| 11 | 2.3 Integrate ReasoningLogger into SecOps Guard | ✅ Done | agents/secops_guard.py missing |
| 12 | 2.4 Integrate ReasoningLogger into Remediation Architect | ✅ Done | agents/remediation_architect.py missing |
...
| 17 | 4.1 Replace `terraform` with `tflocal` in `mcp_server/aws_janitor_mcp.py` | ✅ Done | mcp_server/aws_janitor_mcp.py missing |
| 18 | 4.2 Replace `terraform` with `tflocal` in `hooks/pre-remediation.sh` | ✅ Done | agents/remediation_architect.py missing |
...
| 21 | 4.5 Wire `tflocal apply -auto-approve` into orchestrator approval flow | ✅ Done | "APPROVE" not found in codebase |
...
| 35 | 9. Implement Streamlit Reasoning Panel | ✅ Done | app.py missing |
| 36 | 9.1 Add reasoning log panel to `app.py` | ✅ Done | app.py missing |
...
|---------|--------|---------------|
| FinOps Auditor | Pending | Multi-agent FinOps scanner detecting idle resources, classifying severity, writing findings |
| SecOps Guard | Pending | Security vulnerability scanner — SG rules, encryption, auth checks |
| Remediation Architect | Pending | HCL generation with dependency checks, rollback HCL alongside remediation |
| Agent Orchestrator | Pending | Pipeline sequencing: FinOps → SecOps → Remediation with hooks and approval |
| Approval Gate (persistent, rate-limited) | Pending | 3-attempt lockout, atomic persistence, survives process restarts |
| Rollback with Terraform Execution | Pending | 2-step rollback (ROLLBACK + CONFIRM), TF validate → apply |
| TF_CMD Allowlist Validation | Pending | Binary allowlist, path separator rejection, PATH resolution |
| Pre-Remediation Hook Validation | Pending | Validates rollback files for all active plans with 60s timeout |
| Findings Store Schema Versioning | Pending | Semantic version field in findings_store.json, major version validation |
| Path Convention Alignment (`core/paths.py`) | Pending | Single source of truth for all artifact paths, used by Orchestrator and UI |
| Structured Error Telemetry | Pending | JSONL error records with category, agent_name, traceback (max 4096 chars) |
| Reasoning Log (append-mode, rotation) | Pending | Append-mode with JSONL separator per run, 10MB rotation, 5 file max |
| Savings Tracker | Pending | Ledger lifecycle, duplicate prevention, broad exception handling in orchestrator |
| JanitorScheduler (cron-based scans) | Complete | APScheduler daemon thread, overlap prevention, cron configurable |
| QueryInterpreter | Pending | `agents/query_interpreter.py` — LLM-powered agent with safe defaults |
| RemediationExplainer | Pending | `agents/explainer.py` — LLM-powered agent with safe defaults |
| PolicySuggester | Pending | `agents/policy_suggester.py` — LLM-powered agent with safe defaults |
| ResourceTagger | Pending | `agents/tagger.py` — LLM-powered agent with safe defaults |
| AnomalyDetector | Pending | `agents/anomaly_detector.py` — LLM-powered agent with safe defaults |
| IncidentPolicyGenerator | Pending | `agents/incident_policy_generator.py` — LLM-powered agent with safe defaults |
| DriftDetector | Pending | `agents/drift_detector.py` — LLM-powered agent with safe defaults |
| MultiAccountOrchestrator | Pending | `agents/multi_account_orchestrator.py` — LLM-powered agent with safe defaults |
| NL Audit | Pending | Neither UI nor backend implemented |
| MCP Server (core tools) | Pending | 4 core + 6 AI tools via FastMCP, provider-agnostic backend |
| FixtureProvider | Pending | Reads fixture JSONs, filters resources, computes cost totals |
| AWSProvider | Pending | Not implemented |
| GCPProvider | Pending | Raises NotImplementedError with WARNING on init (intentional stub) |
| AzureProvider | Pending | Raises NotImplementedError with WARNING on init (intentional stub) |
| Streamlit Dashboard | Pending | Agent feed, findings, diff view, approval input, savings counter, reasoning panel |
| pyproject.toml (packaging) | Complete | Hatchling build system, dependencies, dev group, entry points |
| Structured Logging (`logging_config.py`) | Pending | Env-var driven level, ISO 8601 timestamps, stderr output |
...
|------|------|---------|-------|
| Audit Remediation | 1.1 Create `core/paths.py` centralized path configuration with directory creatio | done | `core/paths.py` referenced but not found |
| Audit Remediation | 1.2 Create `core/error_telemetry.py` structured error module | done | `core/error_telemetry.py` referenced but not found |
...
| Cloud Janitor Phase Bc | 1.1 Create `core/llm_client.py` | done | `core/llm_client.py` referenced but not found |
| Cloud Janitor Phase Bc | 2.1 Implement `agents/query_interpreter.py` | done | `agents/query_interpreter.py` referenced but not found |
| Cloud Janitor Phase Bc | 2.3 Implement `agents/explainer.py` | done | `agents/explainer.py` referenced but not found |
| Cloud Janitor Phase Bc | 2.5 Implement `agents/policy_suggester.py` | done | `agents/policy_suggester.py` referenced but not found |
| Cloud Janitor Phase Bc | 3.1 Implement `agents/tagger.py` | done | `agents/tagger.py` referenced but not found |
| Cloud Janitor Phase Bc | 3.3 Implement `agents/anomaly_detector.py` | done | `agents/anomaly_detector.py` referenced but not found |
| Cloud Janitor Phase Bc | 5.1 Implement `agents/incident_policy_generator.py` | done | `agents/incident_policy_generator.py` referenced but not found |
| Cloud Janitor Phase Bc | 5.3 Implement `agents/drift_detector.py` | done | `agents/drift_detector.py` referenced but not found |
| Cloud Janitor Phase Bc | 6.1 Implement `agents/multi_account_orchestrator.py` | done | `agents/multi_account_orchestrator.py` referenced but not found |
| Provider Agnostic Backend | 1.1 Create `mcp_server/backends/__init__.py` with CloudProvider abstract base cl | done | `mcp_server/backends/__init__.py` referenced but not found |
| Provider Agnostic Backend | 1.2 Implement FixtureProvider in `mcp_server/backends/fixture_provider.py` | done | `mcp_server/backends/fixture_provider.py` referenced but not found |
| Provider Agnostic Backend | 2.1 Implement AWSProvider in `mcp_server/backends/aws_provider.py` | done | `mcp_server/backends/aws_provider.py` referenced but not found |
| Provider Agnostic Backend | 2.2 Implement GCPProvider and AzureProvider in `mcp_server/backends/gcp_provider | done | `mcp_server/backends/gcp_provider.py` referenced but not found |
| Provider Agnostic Backend | 2.2 Implement GCPProvider and AzureProvider in `mcp_server/backends/gcp_provider | done | `mcp_server/backends/azure_provider.py` referenced but not found |
| Savings Tracker Localstack | 1.1 Create `agents/savings_tracker.py` with SavingsTracker class | done | `agents/savings_tracker.py` referenced but not found |
| Savings Tracker Localstack | 2.1 Create `agents/reasoning_logger.py` with ReasoningLogger class | done | `agents/reasoning_logger.py` referenced but not found |
