...
Generated: 2026-07-04T17:32:11Z

**Summary:** 232 tasks — ✅ 232 done, ⏳ 0 partial, ❌ 0 pending
...
| 25 | 6. Implement pre-remediation hook full validation | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 26 | 6.1 Implement `_run_pre_remediation_hook_full()` method | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 27 | 6.2 Write property tests for pre-remediation hook | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 36 | 8. Implement savings tracker broad exception handling | ✅ Done | src/cloud_janitor/agents/savings_tracker.py exists |
| 37 | 8.1 Wrap `savings_tracker.record_run()` with broad exception handling | ✅ Done | src/cloud_janitor/agents/savings_tracker.py exists |
| 38 | 8.2 Write property test for savings tracker exception swallowing | ✅ Done | src/cloud_janitor/agents/savings_tracker.py exists |
| 39 | 9. Checkpoint - Ensure all tests pass | ✅ Done | no mapping |
| 40 | 10. Implement UI–Orchestrator contract alignment | ✅ Done | src/cloud_janitor/app.py exists |
| 41 | 10.1 Refactor `app.py` audit delegation to use public Orchestrator API only | ✅ Done | src/cloud_janitor/app.py exists |
| 42 | 10.2 Write unit tests for UI delegation (`tests/test_ui_delegation.py`) | ✅ Done | src/cloud_janitor/app.py exists |
...
| 47 | 10.7 Update `app.py` to import paths from `core/paths.py` | ✅ Done | src/cloud_janitor/app.py exists |
...
| 52 | 11.3 Surface structured error fields in Streamlit UI | ✅ Done | src/cloud_janitor/app.py exists |
...
| 4 | 4. Write fixture JSON for Cost Explorer (3 resources, 2 flaggable) | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 5 | 5. Write fixture JSON for Config/Inspector (2 security findings) | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 6 | 1. Implement aws_janitor_mcp.py with MCP protocol | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 7 | 2. Implement get_cost_data() → reads Cost Explorer fixture | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 8 | 3. Implement get_security_data() → reads Inspector fixture | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 9 | 4. Implement validate_hcl() → shells to terraform validate | ✅ Done | no mapping |
| 10 | 5. Write mcp_server/README.md | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 11 | 1. FinOps Auditor — calls MCP, produces findings[], writes findings_store.json | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 12 | 2. SecOps Guard — calls MCP, appends to findings_store.json | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 13 | 3. Remediation Architect — reads findings, dependency check, generates HCL | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 14 | 4. Rollback HCL generation (alongside remediation, not after) | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 15 | 5. findings_store.json schema validation | ✅ Done | output/findings_store.json exists |
| 16 | 1. pre-remediation.sh — terraform validate gate | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 17 | 2. post-remediation.sh — audit.log append | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 25 | 3. Side-by-side diff view (remediation HCL vs rollback HCL) | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
| 26 | 4. Approval input field + confirmation display | ✅ Done | no mapping |
| 27 | 5. Savings counter | ✅ Done | no mapping |
| 28 | 1. End-to-end Ghost Cluster scenario run (no errors) | ✅ Done | no mapping |
| 29 | 2. Rollback flow run (no errors) | ✅ Done | no mapping |
| 30 | 3. Error state test: approval typo rejected gracefully | ✅ Done | "APPROVE" found in codebase |
| 31 | 4. Rehearse 6-min demo script 3x | ✅ Done | no mapping |
| 32 | 5. Record demo video for Devpost submission | ✅ Done | no mapping |
| 33 | 6. Write Devpost submission copy | ✅ Done | no mapping |
...
| 6 | 2. Implement Phase B AI agents (QueryInterpreter, RemediationExplainer, PolicySuggester) | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 10 | 2.4 Write property test for RemediationExplainer schema completeness | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 30 | 8. Wire MCP tools and orchestrator integration | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 31 | 8.1 Add MCP tool `interpret_query` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 32 | 8.2 Add MCP tool `explain_remediation` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 33 | 8.3 Add MCP tool `suggest_policies` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 34 | 8.4 Add MCP tool `infer_resource_context` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 35 | 8.5 Add MCP tool `detect_anomalies` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 36 | 8.6 Add MCP tool `policy_from_incident` to `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 37 | 8.7 Integrate AI agents into `orchestrator.py` | ✅ Done | no mapping |
| 38 | 8.8 Write unit tests for MCP tools (Phase B+C) | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 39 | 9. Implement fixture mode compatibility | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 40 | 9.1 Update fixture provider for Phase B+C features | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 41 | 9.2 Create `accounts.example.json` template for multi-account testing | ✅ Done | no mapping |
| 42 | 9.3 Write integration tests for fixture mode | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 43 | 10. Implement Streamlit UI integration | ✅ Done | src/cloud_janitor/app.py exists |
| 44 | 10.1 Add NL query input and AI panels to `app.py` | ✅ Done | src/cloud_janitor/app.py exists |
...
| 7 | 2. Checkpoint — Verify Batch 1 | ✅ Done | no mapping |
...
| 23 | 7. Batch 3 — Package structure migration and CI | ✅ Done | no mapping |
...
| 2 | 1.1 Create `mcp_server/backends/__init__.py` with CloudProvider abstract base class | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 3 | 1.2 Implement FixtureProvider in `mcp_server/backends/fixture_provider.py` | ✅ Done | src/cloud_janitor/fixtures/ exists |
| 4 | 1.3 Write property tests for FixtureProvider | ✅ Done | no mapping |
| 5 | 2. Implement stub providers | ✅ Done | no mapping |
| 6 | 2.1 Implement AWSProvider in `mcp_server/backends/aws_provider.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 7 | 2.2 Implement GCPProvider and AzureProvider in `mcp_server/backends/gcp_provider.py` and `mcp_server/backends/azure_provider.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 8 | 2.3 Update `mcp_server/backends/__init__.py` to export all providers | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 9 | 3. Wire provider selection into MCP server | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 10 | 3.1 Add PROVIDER_REGISTRY and `_load_provider()` to `aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 11 | 3.2 Refactor MCP tool functions to delegate to provider instance | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
...
| 18 | 6.3 Update `mcp_server/README.md` with provider architecture documentation | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 19 | 6.4 Create `agents/README.md` | ✅ Done | no mapping |
| 20 | 6.5 Create `fixtures/README.md` | ✅ Done | src/cloud_janitor/fixtures/ exists |
...
## Savings Tracker Localstack (44/44)

| # | Task | Status | Artifact Verified |
|---|------|--------|-------------------|
| 1 | 1. Implement Savings Tracker core module | ✅ Done | no mapping |
| 2 | 1.1 Create `agents/savings_tracker.py` with SavingsTracker class | ✅ Done | src/cloud_janitor/agents/savings_tracker.py exists |
| 3 | 1.2 Write property test: RunEntry schema and field correctness | ✅ Done | no mapping |
| 4 | 1.3 Write property test: Monthly savings computation | ✅ Done | src/cloud_janitor/agents/savings_tracker.py exists |
...
| 10 | 2.2 Integrate ReasoningLogger into FinOps Auditor | ✅ Done | src/cloud_janitor/agents/finops_auditor.py exists |
| 11 | 2.3 Integrate ReasoningLogger into SecOps Guard | ✅ Done | src/cloud_janitor/agents/secops_guard.py exists |
| 12 | 2.4 Integrate ReasoningLogger into Remediation Architect | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 17 | 4.1 Replace `terraform` with `tflocal` in `mcp_server/aws_janitor_mcp.py` | ✅ Done | src/cloud_janitor/mcp_server/aws_janitor_mcp.py exists |
| 18 | 4.2 Replace `terraform` with `tflocal` in `hooks/pre-remediation.sh` | ✅ Done | src/cloud_janitor/agents/remediation_architect.py exists |
...
| 35 | 9. Implement Streamlit Reasoning Panel | ✅ Done | src/cloud_janitor/app.py exists |
| 36 | 9.1 Add reasoning log panel to `app.py` | ✅ Done | src/cloud_janitor/app.py exists |
...
| 44 | 10.5 Run scripts/generate_spec_compliance.py and commit output | ✅ Done | no mapping |
...
| QueryInterpreter | Complete | `src/cloud_janitor/agents/query_interpreter.py` — LLM-powered agent with safe defaults |
| RemediationExplainer | Complete | `src/cloud_janitor/agents/explainer.py` — LLM-powered agent with safe defaults |
| PolicySuggester | Complete | `src/cloud_janitor/agents/policy_suggester.py` — LLM-powered agent with safe defaults |
| ResourceTagger | Complete | `src/cloud_janitor/agents/tagger.py` — LLM-powered agent with safe defaults |
| AnomalyDetector | Complete | `src/cloud_janitor/agents/anomaly_detector.py` — LLM-powered agent with safe defaults |
| IncidentPolicyGenerator | Complete | `src/cloud_janitor/agents/incident_policy_generator.py` — LLM-powered agent with safe defaults |
| DriftDetector | Complete | `src/cloud_janitor/agents/drift_detector.py` — LLM-powered agent with safe defaults |
| MultiAccountOrchestrator | Complete | `src/cloud_janitor/agents/multi_account_orchestrator.py` — LLM-powered agent with safe defaults |
...
| src-layout Package Structure | Complete | src-layout with hatchling build system |
